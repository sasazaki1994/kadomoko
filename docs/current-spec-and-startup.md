...
KadoMoco v0.9 では、作業を邪魔せずに短い区切りを作る「いっしょに集中」を追加します。

- 右クリックメニューから 10 分または 25 分を選び、すぐに集中時間を始められます。
- 集中中は通常のペット表示を保ち、画面上部の小さな残り時間だけを表示します。残り時間を押すと詳細へ戻れ、自発的な会話・発見・小さな遊びは完了まで静かになります。
- 完了時は短い吹き出しと小さな反応だけを表示し、最初の 1 日 3 回には機嫌 +3 / なつき +1 / EXP +3 の控えめな報酬があります。
- 途中で終えてもペナルティや失敗表現はなく、その日の完了回数にも含めません。
- 開始時刻と終了時刻を保存するため、アプリの再起動や PC のスリープをまたいだ場合も終了時刻を過ぎていれば一度だけ完了します。
- セーブデータは v9 に上げ、旧データには `pet.focusSessions` を安全に補います。
