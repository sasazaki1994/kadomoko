...
KadoMoco v0.8 では、睡眠に小さな意味を足す「夢のかけら」を追加します。世話の義務を増やさず、眠っている時間が少しだけ楽しみになることを狙います。

- ペットが眠っている間、低確率(通常 0.14/分)で夢が静かに形成されます。ユーザーへの通知はありません。
- 目覚めると夢は「かけら」として浮上し、ペットの近くに小さな三日月の合図が表示されます。
- 浮上中は右クリックメニューに文脈アクション「夢のはなしを聞く」が現れ、聞くと短い吹き出し・小さな報酬(機嫌+3 / なつき+1 / EXP+2)・短いエピソードが残ります。
- 聞かずに 15〜30 分経つと、かけらは静かに「聞けなかった記録」として夢の記録に残ります。ペナルティやネガティブな表示はありません。
- 夢のテーマは 6 種で、季節・時間帯・空腹・発見や合図の有無・性格から重み付けされます(例: 空腹だと「ちいさなごちそう」の夢を見やすい)。
- 夢は 1 日最大 2 回・最短 2 時間間隔で、スコアや収集率は表示しません。
- 記録パネルに「夢のかけら」セクションを追加し、直近 3 件の夢のテキストを表示します。
- セーブデータは v7 に上げ、旧データには `pet.dreams` を安全に補います。
